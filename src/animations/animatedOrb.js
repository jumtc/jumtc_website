...
export default function OrbEffect() {
  ...
  useEffect(() => {
    ...
    const spheres = geometries.map(
      (geometry, i) => new THREE.Mesh(geometry, materials[i])
    );
      ...
      geometries.forEach((geometry, index) => {
        ...
        for (let i = 0; i < positions.length; i += 3) {
          ...
          const color = baseColor
            .clone()
            .lerp(displacedColor, Math.abs(displacement));
          colors[i] = color.r;
          colors[i + 1] = color.g;
          colors[i + 2] = color.b;
        }
  ...
  return (
    <div
      ...
      style={{
        ...
        filter: "blur(0.5px)",
        background:
          "radial-gradient(circle, rgba(0,140,255,0.3), rgba(0,0,50,0))",
      }}
    />
  );
}
